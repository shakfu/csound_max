...
static void csound6_dsp64(t_csound6 *x, t_object *dsp64, short *count,
                          double samplerate, long maxvectorsize, long flags);
static void csound6_perform64(t_csound6 *x, t_object *dsp64, double **ins,
...
static void csound6_register_controls(t_csound6 *x, t_symbol *s, int argc,
                                      t_atom *argv);
static void in_channel_value_callback(CSOUND *csound, const char *name,
...
static void csound6_set_channel(t_csound6 *x, t_symbol *s, int argc,
                                t_atom *argv);

static void message_callback(CSOUND *, int attr, const char *format,
                             va_list valist);
static void csound6_out_msg_cb(t_csound6 *x);

static void csound6_outchannel_set(t_csound6 *x, t_symbol *s, int argc,
                                   t_atom *argv);
static void csound6_out_chan_cb(t_csound6 *x);
static void csound6_send_channel_output(out_channel *chan, t_csound6 *x);

static void csound6_table_write(t_csound6 *x, t_symbol *s, int argc,
                                t_atom *argv);
static void csound6_table_to_buffer(t_csound6 *x, t_symbol *s, int argc,
                                    t_atom *argv);
static void csound6_buffer_to_table(t_csound6 *x, t_symbol *s, int argc,
                                    t_atom *argv);
// static void csound6_table_read(t_csound6 *x, t_symbol *s, int argc, t_atom
// *argv);
...
const char *csound6_get_fullpath(const char *file_arg) {
    ...
    if (locatefile_extended(filename, &path_id, &outtype, &filetype,
...
void ext_main(void *r) {
    t_class *c =
        class_new("csound6~", (method)csound6_new, (method)csound6_free,
    ...
    class_addmethod(c, (method)csound6_table_to_buffer, "table->buffer",
                    A_GIMME, 0);
    class_addmethod(c, (method)csound6_table_to_buffer, "t->b", A_GIMME, 0);
    class_addmethod(c, (method)csound6_buffer_to_table, "buffer->table",
                    A_GIMME, 0);
    class_addmethod(c, (method)csound6_buffer_to_table, "b->t", A_GIMME, 0);

    class_addmethod(c, (method)csound6_outchannel_set, "outchannel", A_GIMME,
                    0);
    // class_addmethod(c, (method) csound6_outchannel_set, "outputchannels",
    // A_INT, 0);

    // LEGACY control stuff, NB this was registered for the 'set' message in the
    // PD version and called csoundapi_channel, but we are initializing with the
    // "controls" message
    class_addmethod(c, (method)csound6_register_controls, "controls", A_GIMME,
                    0);
    class_addmethod(c, (method)csound6_control, "control", A_DEFSYM, A_DEFFLOAT,
                    0);
...
void *csound6_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    x->score_done_clock =
        clock_new((t_object *)x, (method)csound6_score_done_cb);
    ...
    linklist_flags(
        x->out_msg_list,
        OBJ_FLAG_MEMORY); // <-- use sysmem_freeptr() to free items in the list
...
static void csound6_init_csound(t_csound6 *x, bool is_initial_compile) {
    ...
    if (x->csd_file) {
        ...
        if (x->csd_fullpath) {
            ...
            x->compiled =
                csoundCompile(x->csound, 2, (const char **)cs_cmdl) == 0
                    ? true
                    : false;
        }
        ...
        if (x->orc_fullpath && x->sco_fullpath) {
            const char *cs_cmdl[] = {"csound", x->orc_fullpath,
                                     x->sco_fullpath};
            x->compiled =
                csoundCompile(x->csound, 3, (const char **)cs_cmdl) == 0
                    ? true
                    : false;
        }
    }
    ...
    // number of inlets and outlets can only be set at object creation, not on
    // recompile
    if (is_initial_compile) {
        x->chans = csoundGetNchnls(x->csound);
        x->numlets = x->chans;
        // post("x->chans: %i, x->ksmps %i, x->numlets: %i", x->chans, x->ksmps,
        // x->numlets); create a signal inlet and outlet for each csound channel
        dsp_setup(
            (t_pxobject *)x,
            x->numlets); // MSP inlets: arg is # of inlets and is REQUIRED!
        for (int i = 0; i < x->numlets && i < CS_MAX_CHANS; i++) {
            outlet_new(x, "signal");
        }
    } else if (x->chans != csoundGetNchnls(x->csound)) {
        // recompile changed number of channels, so we will pretend we are not
        // compiled
...
static bool csound6_check_ready(t_csound6 *x) {
    ...
    if (!x->compiled || x->errors == true) {
        // for the purpose of messages, we pretend illegal compliation
        // (x->errors) is not compiled
    ...
    if (x->chans != csoundGetNchnls(x->csound)) {
        post("csound6~ error: number of channels can only be changed on object "
             "creation file load.");
        return false;
    }
    if (x->max_vector_size % x->ksmps != 0) {
        post("csound6~ error: ksmps must be even divisor of Max signal vector "
             "size");
        return false;
    } else {
...
static void csound6_event(t_csound6 *x, t_symbol *s, int argc, t_atom *argv) {
    ...
    // if, in converting, we find a symbol atom, we abort making an array of
    // floats t events always use string events
    ...
    if (use_string_event) {
        ...
        if (err == MAX_ERR_NONE && size && atoms_as_text) {
            ...
            object_error((t_object *)x,
                         "csound6~ error processing score event");
        }
    } else {
        int res = csoundScoreEvent(x->csound, evt_type->s_name[0], pfields,
                                   num_pfields);
    }
        ...
        for (int kpass = 0; kpass < x->kpasses_per_vector; kpass++) {
            ...
            if ((x->end = csoundPerformKsmps(x->csound))) {
                ...
                for (int chan = 0; chan < x->numlets; chan++) {
                    for (int s = 0; s < x->ksmps; s++) {
                        dest_sample_index = kpass_sample_offset + s;
                        outs[chan][dest_sample_index] =
                            csoundGetSpoutSample(x->csound, s, chan);
                    }
                }
            }
...
static void message_callback(CSOUND *csound, int attr, const char *format,
                             va_list valist) {
    int i;
...
static int set_channel_value(t_csound6 *x, t_symbol *channel, MYFLT value) {
    ...
    // traverse the linked list of channels to find the right channel and set
    // it's value
    if (ch == NULL)
        return 0;
    ...
    ch->value = value;
    // post(" - found channel, has set %s to %.2f", ch->name->s_name,
    // ch->value);
    return 1;
}
...
static channelname *create_channel(channelname *ch, char *channel) {
    ...
    channelname *tmp = ch,
                *newch = (channelname *)sysmem_newptr(sizeof(channelname));
...
static void out_channel_value_callback(CSOUND *csound, const char *name,
    ...
    // TODO: this is working, but it's not going to be optimal in terms of
    // performance
...
static void csound6_set_channel(t_csound6 *x, t_symbol *s, int argc,
    ...
    for (i = 0; i < argc; i += 2) {
        ...
        if (i + 1 < argc) {
            ...
            case A_SYM:
                csoundSetStringChannel(csound, chan_name,
                                       atom_getsym(&argv[i + 1])->s_name);
                break;
            case A_LONG:
            case A_FLOAT:
...
/*********************************************************************************/
// out channel code for direct monitoring (not the same as outvalue legacy
// system)
...
static out_channel *csound6_get_out_channel(t_csound6 *x, char *channel_name) {
    void *chan_entry;
    long index;
    index = linklist_findfirst(x->out_channels, &chan_entry, outchan_cmp,
                               (void *)channel_name);
    if (index != -1) {
        // post("index: %i, item found: %s", index, ((out_channel
        // *)chan_entry)->name);
...
static void csound6_outchannel_set(t_csound6 *x, t_symbol *s, int argc,
    ...
    if (argc != 2 || atom_gettype(argv) != A_SYM ||
        ...
        post("csound6~ error: outchannel requires 2 arguments of symbol and "
             "int");
        return;
    }
    ...
    if (chan_entry && update_arg <= 0) {
        ...
        if (x->post_messages)
            post("csound6~ output kchan %s set to %i ksmps", chan_entry->name,
                 chan_entry->update);
    } else if (chan_entry == NULL && update_arg > 0) {
        // register new channel to listen to
        out_channel *chan_reg =
            (out_channel *)sysmem_newptr(sizeof(out_channel));
        strcpy(chan_reg->name, chan_name);
        chan_reg->update = (int)atom_getlong(argv + 1);
        linklist_append(x->out_channels, chan_reg);
        if (x->post_messages)
            post("csound6~ output kchan %s set to %i ksmps", chan_reg->name,
                 chan_reg->update);
    } else if (chan_entry == NULL && update_arg <= 0) {
        // post(" - updating existing entry update to %i", update_arg);
        if (x->post_messages)
...
static void csound6_send_channel_output(out_channel *chan, t_csound6 *x) {
    // post("csound6_send_channel_output() channel: %s update: %i", chan->name,
    // chan->update);
    int error;
    t_atom out_atoms[3];
    ...
    if (chan->update != 0) {
        if (chan->kpass == 0) {
            double chan_val =
                csoundGetControlChannel(x->csound, chan->name, &error);
            // post("csound6_send_channel_output() chan: %s  value: %f  error:
            // %i", chan->name, chan_val, error);
...
static void csound6_table_write(t_csound6 *x, t_symbol *s, int argc,
    ...
    if (table_length == -1) {
        ...
        for (int i = 0; i < num_values; i++) {
            ...
            // post("updated table: %i index: %i value: %f", table, index + i,
            // value);
        }
    }
}
...
static void csound6_table_to_buffer(t_csound6 *x, t_symbol *s, int argc,
    ...
    if (argc < 2 || atom_gettype(argv) != A_LONG ||
        atom_gettype(argv + 1) != A_SYM) {
        post("csound6~ error: tab->buff requires 2 arguments: table number, "
             "buffer name");
        return;
    }
    ...
    if (table_size == -1) {
        object_error((t_object *)x,
                     "Csound table %s not found, is csound running?",
                     table_num);
        return;
    }
    ...
    if (buffer == NULL) {
        object_error((t_object *)x, "Unable to reference buffer named %s",
                     buffer_name->s_name);
        return;
    }
    ...
    if (buff_frames == 0) {
        object_error((t_object *)x, "Buffer %s is 0 points in size, aborting.",
                     buffer_name->s_name);
        return;
    }
...
static void csound6_buffer_to_table(t_csound6 *x, t_symbol *s, int argc,
    ...
    if (argc < 2 || atom_gettype(argv) != A_SYM ||
        atom_gettype(argv + 1) != A_LONG) {
        post("csound6~ error: buff->tab requires 2 arguments: buffer name, "
             "table number");
        return;
    }
